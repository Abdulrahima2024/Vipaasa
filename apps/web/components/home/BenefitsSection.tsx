...
export default function BenefitsSection() {
  const benefits = [
    {
      icon: (
        <svg className="w-8 h-8 text-[#00C853]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <section className="py-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {benefits.map((b, index) => (
          <div
            key={index}
            className="group relative overflow-hidden bg-white/50 border border-[#EAE6DB]/60 rounded-3xl p-6 sm:p-8 flex flex-col items-center text-center space-y-4 shadow-[0_8px_30px_rgba(0,0,0,0.01)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.03)] hover:border-[#00C853]/30 hover:bg-white transition-all duration-500 transform hover:-translate-y-1.5"
          >
            {/* Glow Background Effect */}
            <div className="absolute top-0 right-0 w-24 h-24 bg-[#00C853]/5 rounded-full blur-2xl group-hover:bg-[#00C853]/10 transition-all duration-500 -mr-6 -mt-6 pointer-events-none" />

            {/* Centered Icon Container with Background Accent */}
            <div className="w-16 h-16 bg-[#EAF5EC] rounded-2xl flex items-center justify-center group-hover:scale-105 group-hover:bg-[#00C853]/10 transition-all duration-300">
              {b.icon}
            </div>

            {/* Centered Heading */}
            <h4 className="font-sans text-base sm:text-lg font-bold text-[#00C853] tracking-tight leading-snug group-hover:text-[#00a142] transition-colors duration-300">
              {b.title}
            </h4>
