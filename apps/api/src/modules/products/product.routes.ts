...
router.post("/products", authenticate, authorize(["ADMIN", "SUPER_ADMIN"]), upload.array("images", 3), createProduct);

// Delete a product (Admin only)
router.delete("/products/:id", authenticate, authorize(["ADMIN", "SUPER_ADMIN"]), deleteProduct);

// Update a product (Admin only)
router.patch("/products/:id", authenticate, authorize(["ADMIN", "SUPER_ADMIN"]), upload.array("images", 3), updateProduct);
